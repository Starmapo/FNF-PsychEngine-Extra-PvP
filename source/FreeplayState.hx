...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			for (song in leWeek.songs)
			{
				...
				addSong(song[0], i, song[1], FlxColor.fromRGB(colors[0], colors[1], colors[2]), Song.getDisplayName(song[0]),
					(meta.iconHiddenUntilPlayed == true));
			}
		}
		...
		songs = songsOG.copy();
		if ((curSelected >= songs.length) || (fromPlayState && lastSort == 'Last Played'))
			curSelected = 0;
		...
		var sortTypes = ['Week (Default)', 'Name', 'Last Played'];
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		if (newPos < 0)
			newPos = CoolUtil.difficulties.indexOf(lastDifficultyName.charAt(0).toUpperCase() + lastDifficultyName.substr(1));
		if (newPos < 0)
			newPos = CoolUtil.difficulties.indexOf(lastDifficultyName.toLowerCase());
		if (newPos < 0)
			newPos = CoolUtil.difficulties.indexOf(lastDifficultyName.toUpperCase());
		if (newPos > -1)
		{
	...
	function setListFromSearch(text:String = '', sort:String = '', change:Bool = true)
	{
		...
		else
		{
			...
			var daOtherSongs = songsOG.filter(song -> (!song.displayName.toLowerCase().startsWith(daText)
				&& song.displayName.toLowerCase().contains(daText)));
			if (sort == 'Name')
	...
	function regenMenu(change:Bool = true)
	{
		...
		for (i in 0...songs.length)
		{
			...
			if (250 + songText.width > FlxG.width)
			{
				var textScale:Float = 390 / songText.width;
				songText = new Alphabet(0, (70 * i) + 30, songs[i].displayName, true, false, textScale);
			}
