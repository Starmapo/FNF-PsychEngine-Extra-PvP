...
class CoolUtil
{
	...
	public static function playPvPMusic(volume:Float = 1) {
		FlxG.sound.playMusic(Paths.music('pvp/${FlxG.random.int(1, 10)}'), volume * ClientPrefs.menuMusicVolume);
	}
	...
	public static function existsVoices(song:String) {
		song = Paths.formatToSongPath(song);
		return Paths.existsPath('$song/Voices.${Paths.SOUND_EXT}', MUSIC, 'songs') || Paths.existsPath('$song/Voices${CoolUtil.getDifficultyFilePath()}.${Paths.SOUND_EXT}', MUSIC, 'songs');
	}
