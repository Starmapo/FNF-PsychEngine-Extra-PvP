...
class PlayState extends MusicBeatState
{
		...
		if (!inEditor && isStoryMode && !seenCutscene)
		{
			switch (curSong)
			{
				case "monster":
					...
					tweenManager.tween(whiteScreen, {alpha: 0}, 1, {
						startDelay: 0.1,
						ease: FlxEase.linear,
				...
				case "winter-horrorland":
					...
					tweenManager.tween(blackScreen, {alpha: 0}, 0.7, {
						ease: FlxEase.linear,
						onComplete: function(twn:FlxTween)
						{
							remove(blackScreen);
	...
	function tankIntro()
	{
		...
		cutsceneHandler.finishCallback = function()
		{
			...
			tweenManager.tween(FlxG.camera, {zoom: defaultCamZoom}, timeForStuff, {ease: FlxEase.quadInOut});
			moveCamera(true);
			startCountdown();
		...
		switch (songName)
		{
			...
			case 'guns':
				...
				{
					tightBars.play(true);
					tweenManager.tween(FlxG.camera, {zoom: defaultCamZoom * 1.2}, 4, {ease: FlxEase.quadInOut});
					tweenManager.tween(FlxG.camera, {zoom: defaultCamZoom * 1.2 * 1.2}, 0.5, {ease: FlxEase.quadInOut, startDelay: 4});
					tweenManager.tween(FlxG.camera, {zoom: defaultCamZoom * 1.2}, 1, {ease: FlxEase.quadInOut, startDelay: 4.5});
				};
			...
			case 'stress':
				...
				tweenManager.tween(FlxG.camera, {zoom: 0.9 * 1.2}, 1, {ease: FlxEase.quadInOut});
				...
				{
					tweenManager.tween(camFollow, {x: 650, y: 300}, 1, {ease: FlxEase.sineOut});
					tweenManager.tween(FlxG.camera, {zoom: 0.9 * 1.2 * 1.2}, 2.25, {ease: FlxEase.quadInOut});
				...
				cutsceneHandler.timer(31.2, function()
				{
					...
					cameraSpeed = 12;
					tweenManager.tween(FlxG.camera, {zoom: 0.9 * 1.2 * 1.2}, 0.25, {ease: FlxEase.elasticOut});
				});
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			startTimer = new FlxTimer(timerManager).start(Conductor.normalizedCrochet / 1000, function(tmr:FlxTimer)
			{
				if (!inEditor)
				{
					if (swagCounter < 4)
				...
				switch (swagCounter)
				{
					...
					case 1:
						...
						tweenManager.tween(countdownReady, {alpha: 0}, Conductor.normalizedCrochet / 1000, {
					...
					case 2:
						...
						tweenManager.tween(countdownSet, {alpha: 0}, Conductor.normalizedCrochet / 1000, {
							ease: FlxEase.cubeInOut,
							onComplete: function(twn:FlxTween)
							{
								remove(countdownSet);
					...
					case 3:
						...
						tweenManager.tween(countdownGo, {alpha: 0}, Conductor.normalizedCrochet / 1000, {
							ease: FlxEase.cubeInOut,
							onComplete: function(twn:FlxTween)
							{
								remove(countdownGo);
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if (ClientPrefs.scoreZoom && !miss && !cpuControlled)
		{
			...
			scoreTxtTween = tweenManager.tween(scoreTxt.scale, {x: 1, y: 1}, 0.2, {
				onComplete: function(twn:FlxTween)
				{
					scoreTxtTween = null;
				}
	...
	function startSong():Void
	{
		...
		if (!inEditor)
		{
			tweenManager.tween(timeBar, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
		...
		if (ClientPrefs.keybindReminders && !cpuControlled)
		{
			...
			for (i in 0...playerKeys)
			{
				...
				for (j in 0...keybinds.length)
				{
					...
					keybindTweenGroup.set(txt, tweenManager.tween(txt, {alpha: 0}, 0.5, {
	...
	public function moveCamera(isDad:Bool)
	{
		...
		else
		{
			...
			{
				cameraTwn = tweenManager.tween(FlxG.camera, {zoom: 1}, (Conductor.normalizedCrochet / 1000), {
	...
	function tweenCamIn()
	{
		...
		{
			cameraTwn = tweenManager.tween(FlxG.camera, {zoom: 1.3}, (Conductor.normalizedCrochet / 1000), {
				ease: FlxEase.elasticInOut,
				onComplete: function(twn:FlxTween)
				{
					cameraTwn = null;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			tweenManager.tween(numScore, {alpha: 0}, 0.2, {
		...
		tweenManager.tween(comboSpr, {alpha: 0}, 0.2, {
			onComplete: function(tween:FlxTween)
			{
				coolText.destroy();
				comboSpr.destroy();
	...
	function doRatingTween(ind:Int = 0)
	{
		if (ClientPrefs.scoreZoom)
		{
			...
			ratingTxtTweens[ind] = tweenManager.tween(ratingTxtGroup.members[ind].scale, {x: 1, y: 1}, 0.2, {
				onComplete: function(twn:FlxTween)
				{
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && (inEditor || !playerChar.members[0].stunned) && generatedMusic)
		{
			...
			else if (!inEditor)
			{
				for (char in playerChar)
				{
					if (char.holdTimer > Conductor.normalizedStepCrochet * 0.0011 * char.singDuration && char.state == 'sing')
						char.dance();
				}
			}
		}
			...
			for (char in characters)
			{
					...
					&& !charGroup.members[char].skipSing)
				{
					...
					if (charGroup.members[char].animation.exists(animToPlay))
					{
						...
						charGroup.members[char].state = 'miss';
					}
				}
			}
		}
	...
	override function destroy()
	{
		...
		songSpeedTween = FlxDestroyUtil.destroy(songSpeedTween);
		songSpeedType = null;
		boyfriendGroup = FlxDestroyUtil.destroy(boyfriendGroup);
		dadGroup = FlxDestroyUtil.destroy(dadGroup);
		gfGroup = FlxDestroyUtil.destroy(gfGroup);
		vocals = FlxDestroyUtil.destroy(vocals);
		vocalsDad = FlxDestroyUtil.destroy(vocalsDad);
		unspawnNotes = null;
		eventNotes = null;
		strumLine = FlxDestroyUtil.destroy(strumLine);
		strumLineNotes = FlxDestroyUtil.destroy(strumLineNotes);
		grpNoteSplashes = FlxDestroyUtil.destroy(grpNoteSplashes);
		keybindGroup = FlxDestroyUtil.destroy(keybindGroup);
		curSong = null;
		curSongDisplayName = null;
		healthBarBG = FlxDestroyUtil.destroy(healthBarBG);
		healthBar = FlxDestroyUtil.destroy(healthBar);
		timeBarBG = FlxDestroyUtil.destroy(timeBarBG);
		timeBar = FlxDestroyUtil.destroy(timeBar);
		underlayPlayer = FlxDestroyUtil.destroy(underlayPlayer);
		ratingsData = null;
		botplayTxt = FlxDestroyUtil.destroy(botplayTxt);
		practiceFailedTxt = FlxDestroyUtil.destroy(practiceFailedTxt);
		...
		dialogue = null;
		dialogueJson = null;
		dadbattleBlack = FlxDestroyUtil.destroy(dadbattleBlack);
		dadbattleLight = FlxDestroyUtil.destroy(dadbattleLight);
		dadbattleSmokes = FlxDestroyUtil.destroy(dadbattleSmokes);
		blammedLightsBlack = FlxDestroyUtil.destroy(blammedLightsBlack);
		phillyWindowEvent = FlxDestroyUtil.destroy(phillyWindowEvent);
		phillyGlowGradient = FlxDestroyUtil.destroy(phillyGlowGradient);
		phillyGlowParticles = FlxDestroyUtil.destroy(phillyGlowParticles);
		scoreTxt = FlxDestroyUtil.destroy(scoreTxt);
		timeTxt = FlxDestroyUtil.destroy(timeTxt);
		if (scoreTxtTween != null)
			scoreTxtTween.cancel();
		scoreTxtTween = FlxDestroyUtil.destroy(scoreTxtTween);
		stepTxt = FlxDestroyUtil.destroy(timeTxt);
		beatTxt = FlxDestroyUtil.destroy(timeTxt);
		sectionTxt = FlxDestroyUtil.destroy(timeTxt);
		ratingTxtGroup = FlxDestroyUtil.destroy(ratingTxtGroup);
		ratingTxtTweens = null;
		boyfriendCameraOffset = null;
		opponentCameraOffset = null;
		girlfriendCameraOffset = null;
		#if DISCORD_ALLOWED
		storyDifficultyText = null;
		detailsText = null;
		detailsPausedText = null;
		detailsGameOverText = null;
		#end
		keysPressed = null;
		instance = null;
		luaArray = null;
		luaDebugGroup = FlxDestroyUtil.destroy(luaDebugGroup);
		debugKeysChart = null;
		...
		endingTimer = FlxDestroyUtil.destroy(endingTimer);
		countdownReady = FlxDestroyUtil.destroy(countdownReady);
		countdownSet = FlxDestroyUtil.destroy(countdownSet);
		countdownGo = FlxDestroyUtil.destroy(countdownGo);
		introSoundsSuffix = null;
		noteTypeMap = null;
		eventPushedMap = null;
		keybindTweenGroup = null;
		#if ACHIEVEMENTS_ALLOWED
		achievementObj = FlxDestroyUtil.destroy(achievementObj);
		#end
		ratingName = null;
		ratingFC = null;
		...
		if (!inEditor)
		{
			...
			if (curBeat >= 0)
			{
				...
				for (group in chars)
				{
					...
					{
						if (char.danceEveryNumBeats > 0
							&& curNumeratorBeat % (Math.round(char.danceEveryNumBeats * (Conductor.timeSignature[1] / 4))) == 0 && !char.stunned
								&& char.state == 'default')
							char.dance();
					}
				}
