...
class Stage extends FlxBasic
{
			...
			switch (curStage)
			{
				...
				case 'limo': // Week 4
					...
					if (ClientPrefs.gameQuality == 'Normal')
					{
						...
						grpLimoParticles.scrollFactor.set(0.4, 0.4);
				...
				case 'tank': // Week 7 - Ugh, Guns, Stress
					...
					if (ClientPrefs.gameQuality == 'Normal')
						foreground.add(new BGSprite('tank1', -300, 750, 2, 0.2, ['fg']));
					foreground.add(new BGSprite('tank2', 450, 940, 1.5, 1.5, ['foreground']));
					if (ClientPrefs.gameQuality == 'Normal')
						foreground.add(new BGSprite('tank4', 1300, 900, 1.5, 1.5, ['fg']));
					foreground.add(new BGSprite('tank5', 1620, 700, 1.5, 1.5, ['fg']));
					if (ClientPrefs.gameQuality == 'Normal')
						foreground.add(new BGSprite('tank3', 1300, 1200, 3.5, 2.5, ['fg']));
			...
			switch (curStage)
			{
				case 'spooky':
					if (FlxG.random.bool(10)
						&& instance.curBeat > lightningStrikeBeat + lightningOffset
						&& ClientPrefs.gameQuality != 'Crappy')
					{
						lightningStrikeShit();
					}
				...
				case 'philly':
					...
					if (curNumeratorBeat % (Conductor.timeSignature[0] * 2) == Conductor.timeSignature[0] && FlxG.random.bool(30) && !trainMoving
						&& trainCooldown > 8 && !trainSound.playing)
					{
	...
	function lightningStrikeShit():Void
	{
		...
		if (ClientPrefs.camZooms)
		{
			...
			if (!instance.camZooming)
			{ // Just a way for preventing it to be permanently zoomed until Skid & Pump hits a note
				instance.tweenManager.tween(FlxG.camera, {zoom: instance.defaultCamZoom}, 0.5);
				instance.tweenManager.tween(instance.camHUD, {zoom: 1}, 0.5);
			}
		}
		...
		if (ClientPrefs.flashing)
		{
			...
			instance.tweenManager.tween(halloweenWhite, {alpha: 0.5}, 0.075);
			instance.tweenManager.tween(halloweenWhite, {alpha: 0}, 0.25, {startDelay: 0.15});
		}
	}
	...
	function fastCarDrive()
	{
		...
		carTimer = new FlxTimer(instance.timerManager).start(2, function(tmr:FlxTimer)
		{
			resetFastCar();
			carTimer = null;
		});
