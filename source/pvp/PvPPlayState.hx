...
class PvPPlayState extends MusicBeatState
{
	...
	{
		gamepadControls = new Controls('player2', None);
		gamepadControls.addDefaultGamepad(0);

		for (i in 0...2)
			camGames[i] = new FlxCamera(Std.int((FlxG.width / 2) * i), 0, Std.int(FlxG.width / 2));
		var camPlayerBorder = new FlxCamera(639, 0, 3, FlxG.height);
		...
		for (i in 0...camGames.length)
		{
			...
			{
				daCamFollow = prevCamFollow[i];
				prevCamFollow[i] = null;
			}
			if (prevCamFollowPos[i] != null)
			{
				daCamFollowPos = prevCamFollowPos[i];
		...
		for (player in 0...2)
		{
			var winGraphic = new FlxSprite(20, 410).loadGraphic(Paths.image('pvp/wins$player'), true, 65, 30);
			if (player == 1)
			{
	...
	override public function update(elapsed:Float)
	{
		...
		var pause = controls.PAUSE || gamepadControls.PAUSE;
		var p2Switch = FlxG.keys.justPressed.ALT;
		var p1Switch = gamepadControls.BACK;
		...
		if (ratingPercent[0] > ratingPercent[1])
			health = 0;
		else if (ratingPercent[1] > ratingPercent[0])
			health = 2;
		else
			health = 1;
			...
			for (i in 0...camGames.length)
			{
				if (i == 2)
					camGames[i].zoom = FlxMath.lerp(Math.min(defaultCamZoom, 0.5), camGames[i].zoom,
						CoolUtil.boundTo(1 - (elapsed * 3.125 * camZoomingDecay), 0, 1));
				else
	...
	override function beatHit()
	{
		...
		if (curBeat >= 0 && !endingSong)
		{
			...
			for (group in chars)
			{
				for (char in group)
				{
					var notSinging = (char.state == 'sing'
						&& !FlxG.keys.anyPressed(char.keysPressed)
					...
					if (char.danceEveryNumBeats > 0
						&& curNumeratorBeat % (Math.round(char.danceEveryNumBeats * (Conductor.timeSignature[1] / 4))) == 0 && !char.stunned
							&& char.animation.curAnim != null && (char.state != 'sing' || notSinging))
					{
						char.dance(true);
	...
	public function endSong():Void
	{
		...
		new FlxTimer(timerManager).start(3, function(tmr)
		{
			storyPlaylist.shift();
			...
			else
			{
				...
				if (tiebreakerNext)
				{
					CoolUtil.difficulties = tiebreakerDiffs.copy();
					PlayState.storyDifficulty = CoolUtil.difficulties.length - 1;
				}
				else
				{
					CoolUtil.getDifficulties(storyPlaylist[0], true);
					PlayState.storyDifficulty = storyDifficulties[0];
				}
				PlayState.storyWeek = storyWeeks[0];
	...
	{
		PlayState.SONG.speed = CoolUtil.boundTo(CoolUtil.scrollSpeedFromBPM(PlayState.SONG.bpm, PlayState.SONG.timeSignature[1]), 0.1, 3.5);
		...
		var curDenominator = PlayState.SONG.timeSignature[1];
	...
	private function keyShit():Void
	{
		...
		if (!strumLineNotes.members[1].botPlay)
		{
			for (i in keysArray)
		...
		if (!strumLineNotes.members[0].botPlay)
		{
			// ALL OPPONENT INPUTS HERE
			var controlArray:Array<Bool> = [
				gamepadControls.NOTE_LEFT_P,
				gamepadControls.NOTE_DOWN_P,
				gamepadControls.NOTE_UP_P,
				gamepadControls.NOTE_RIGHT_P
			];
			...
			controlHoldArray = [
				gamepadControls.NOTE_LEFT,
				gamepadControls.NOTE_DOWN,
				gamepadControls.NOTE_UP,
				gamepadControls.NOTE_RIGHT
			];
			if (controlHoldArray.contains(true) && generatedMusic)
			{
				// rewritten inputs???
				strumLineNotes.members[0].holdsGroup.forEachAlive(function(daNote:Note)
				{
					// hold note functions
			...
			var controlReleaseArray:Array<Bool> = [
				gamepadControls.NOTE_LEFT_R,
				gamepadControls.NOTE_DOWN_R,
				gamepadControls.NOTE_UP_R,
				gamepadControls.NOTE_RIGHT_R
			];
	...
	function noteMiss(daNote:Note, player:Int = 0):Void
	...
	private function generateStaticArrows(id:Int = 0, keys:Int = 4, isBoyfriend:Bool = false):Void
	{
		...
		strumGroup.botPlay = #if debug true #else false #end;
	...
	public function recalculateRating(badHit:Bool = false, player:Int = 0)
	{
		...
		if (goods[player] > 0)
			ratingFC[player] = "GFC";
		if (bads[player] > 0 || shits[player] > 0)
			ratingFC[player] = "FC";
		if (songMisses[player] > 0 && songMisses[player] < 10)
			ratingFC[player] = "SDCB";
		else if (songMisses[player] >= 10)
			ratingFC[player] = "Clear";
	...
	public function updateScore(miss:Bool = false, player:Int = 0)
	{
		...
		if (ClientPrefs.showRatings)
			...
			txt = 'Score: ' + Math.round(songScore[player]) + ' | Combo: ' + combo[player] + ' | Fails: ' + songMisses[player] + ' | Rating: '
				+ ratingName[player];
		...
		if (ClientPrefs.scoreZoom && !miss)
		{
			...
			scoreTxtTween[player] = tweenManager.tween(scoreTxt[player].scale, {x: 1, y: 1}, 0.2, {
				onComplete: function(twn:FlxTween)
				{
	...
	function doRatingTween(ind:Int = 0, player:Int = 0)
	{
		if (ClientPrefs.scoreZoom)
		{
			...
			ratingTxtTweens[player][ind] = tweenManager.tween(ratingTxtGroup[player].members[ind].scale, {x: 1, y: 1}, 0.2, {
				onComplete: function(twn:FlxTween)
				{
