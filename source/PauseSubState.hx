...
class PauseSubState extends MusicBeatSubState
{
	...
	{
		grpMenuShit = FlxDestroyUtil.destroy(grpMenuShit);
		menuItems = null;
		menuItemsOG = null;
		difficultyChoices = null;
		pauseMusic = FlxDestroyUtil.destroy(pauseMusic);
		practiceText = FlxDestroyUtil.destroy(practiceText);
	...
	{
		skipTimeText.text = FlxStringUtil.formatTime(Math.max(0, Math.floor(curTime / Conductor.playbackRate / 1000)), false)
			+ ' / '
			+ FlxStringUtil.formatTime(Math.max(0, Math.floor(FlxG.sound.music.length / Conductor.playbackRate / 1000)), false);
	}
}
