...
import flixel.ui.FlxUI;
import flixel.ui.FlxUICheckBox;
import flixel.ui.FlxUIDropDownMenu;
import flixel.ui.FlxUIInputText;
import flixel.ui.FlxUINumericStepper;
import flixel.ui.FlxUITabMenu;
...
class StageEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		var steppers:Array<FlxUINumericStepper> = [
			boyfriendPositionXStepper,
			boyfriendPositionYStepper,
			dadPositionXStepper,
			dadPositionYStepper,
			gfPositionXStepper,
			gfPositionYStepper
	...
	function addStageUI()
	{
		...
		boyfriendPositionYStepper = new FlxUINumericStepper(boyfriendPositionXStepper.x + 60, boyfriendPositionXStepper.y, 10, stageFile.boyfriend[1], -9000,
			9000, 3);
		dadPositionXStepper = new FlxUINumericStepper(boyfriendPositionXStepper.x, boyfriendPositionXStepper.y + 40, 10, stageFile.opponent[0], -9000, 9000,
			3);
		...
		boyfriendCameraYStepper = new FlxUINumericStepper(boyfriendCameraXStepper.x + 60, boyfriendCameraXStepper.y, 10, stageFile.camera_boyfriend[1], -9000,
			9000, 3);
		dadCameraXStepper = new FlxUINumericStepper(boyfriendCameraXStepper.x, boyfriendCameraXStepper.y + 40, 10, stageFile.camera_opponent[0], -9000, 9000,
			3);
	...
	function updatePointerPos()
	{
		if (!gf.visible)
			...
			cameraFollowPointerGF.setPosition(CoolUtil.getCamFollowCharacter(gf).x + stageFile.camera_girlfriend[0],
				CoolUtil.getCamFollowCharacter(gf).y + stageFile.camera_girlfriend[1]);
		cameraFollowPointerDad.setPosition(CoolUtil.getCamFollowCharacter(dad).x + stageFile.camera_opponent[0],
			CoolUtil.getCamFollowCharacter(dad).y + stageFile.camera_opponent[1]);
		cameraFollowPointerBoyfriend.setPosition(CoolUtil.getCamFollowCharacter(boyfriend).x + stageFile.camera_boyfriend[0],
			CoolUtil.getCamFollowCharacter(boyfriend).y + stageFile.camera_boyfriend[1]);
