...
import flixel.on.FlxAnimation;
import flixel.s.FlxGraphic;
import flixel.lxGroup.FlxTypedGroup;
import flixel.debug.interaction.tools.Pointer.GraphicCursorCross;
import flixel.xText;
...
class CharacterEditorState extends MusicBeatState
{
	...
	function addAnimationsUI() {
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Add/Update", function() {
			...
			for(i in 0...char.animationsArray.length) {
				if (char.animationsArray[i] != null && char.animationsArray[i].anim == animationInputText.text) {
					...
					if (leAnim != null && leAnim.frames.length > 0) {
						...
						curAnim = i;
						animationDropDown.selectedLabel = char.animationsArray[i].anim;
						break;
					}
				}
			}
			...
			animationDropDown.selectedLabel = animationDropDown.list[curAnim].label.text;
			genBoyOffsets();
		...
		var removeButton:FlxButton = new FlxButton(180, animationIndicesInputText.y + 30, "Remove", function() {
			for (anim in char.animationsArray) {
				if (animationInputText.text == anim.anim) {
					...
					if (char.animation.curAnim == null || !char.animation.exists(anim.anim) || (char.animation.curAnim != null && anim.anim == char.animation.curAnim.name)) resetAnim = true;
	...
	function reloadCharacterImage() {
		...
		trace(daAnim, char.getImageFileCrop());
	}
