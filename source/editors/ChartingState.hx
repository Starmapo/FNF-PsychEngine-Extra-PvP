...
import flixel.lxGroup;
import flixel.xMath;
import flixel.xPoint;
...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		if (curSec >= _song.notes.length || _song.notes[curSec] == null || lastSong != currentSongName) curSec = 0;
	...
	function addSongUI():Void
	{
		...
		var stepperDadKeys:FlxUINumericStepper = new FlxUINumericStepper(stepperBoyfriendKeys.x, stepperBoyfriendKeys.y + 35, 1, 4, 1, Note.MAX_KEYS);
		...
		var difficultyDropDown = new FlxUIDropDownMenu(stageDropDown.x, gfVersionDropDown.y, FlxUIDropDownMenu.makeStrIdLabelArray(CoolUtil.difficulties, true), function(difficulty:String)
		{
			if (PlayState.storyDifficulty != Std.parseInt(difficulty)) {
				...
				try {
					PlayState.SONG = Song.loadFromJson(Highscore.formatSong(_song.song), _song.song);
					_song = PlayState.SONG;
					LoadingState.loadAndResetState();
				} catch (e) {
					trace('File ${Highscore.formatSong(_song.song)} was not found.');
				}
			}
		});
	...
	function loadSong():Void
	{
		...
		if (CoolUtil.existsVoices(currentSongName)) {
				...
				if (i > 0 && sectionStartTime(i) >= FlxG.sound.music.length) {
					if (_song.notes[i] != null) {
						for (note in _song.notes[i].sectionNotes) {
							_song.notes[i].sectionNotes.remove(note);
							_song.notes[i - 1].sectionNotes.push(note);
						}
					}
	...
	function reloadGridLayer() {
		gridLayer.clear();
		overWaveform.clear();
		trace(curSec, _song.notes.length, _song.notes[curSec]);
	...
	function loadJson(song:String):Void
	{
		...
		PlayState.SONG = Song.loadFromJson(Highscore.formatSong(song), song);
		_song = PlayState.SONG;
		trace('loading: ' + Highscore.formatSong(song));
		LoadingState.loadAndResetState();
	}
	...
	private function saveLevel()
	{
		...
		var savedSong:SwagSong = {
			...
			skinModifier: _song.skinModifier,
			type: 'psychextra'
		};
		var json = {
			"song": savedSong
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), '${Highscore.formatSong(_song.song)}.json');
		}
	}
